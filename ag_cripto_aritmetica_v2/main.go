...
// }
 ...
 func calcIndividualFitness(individual *[VECTOR_SIZE]int) {
 	//EAT + THAT = APPLE
 	var eat, that, apple int
 	eat += individual[E] * 100
 	eat += individual[A] * 10
 	eat += individual[T]

 	that += individual[T] * 1000
 	that += individual[H] * 100
 	that += individual[A] * 10
 	that += individual[T]

 	apple += individual[A] * 10000
 	apple += individual[P] * 1000
 	apple += individual[P] * 100
 	apple += individual[L] * 10
 	apple += individual[E]

 	fitness := (eat + that) - apple
 	if fitness < 0 {
 		fitness = fitness * (-1)
 	}
 	individual[POS_EVAL] = fitness
...
/*func calcIndividualFitness(individual *[VECTOR_SIZE]int) {
	CROSS + ROADS = DANGER
	var cross, roads, danger int
	cross += individual[C] * 10000
	cross += individual[R] * 1000
	cross += individual[O] * 100
	cross += individual[S] * 10
	cross += individual[S]

	roads += individual[R] * 10000
	roads += individual[O] * 1000
	roads += individual[A] * 100
	roads += individual[D] * 10
	roads += individual[S]

	danger += individual[D] * 100000
	danger += individual[A] * 10000
	danger += individual[N] * 1000
	danger += individual[G] * 100
	danger += individual[E] * 10
	danger += individual[R]

	fitness := (cross + roads) - danger
	if fitness < 0 {
		fitness = fitness * (-1)
	}
	individual[POS_EVAL] = fitness
...
// func calcIndividualFitness(individual *[VECTOR_SIZE]int) {
// 	//COCA + COLA = OASIS
// 	var coca, cola, oasis int
// 	coca += individual[C] * 1000
// 	coca += individual[O] * 100
// 	coca += individual[C] * 10
// 	coca += individual[A]
//
// 	cola += individual[C] * 1000
// 	cola += individual[O] * 100
// 	cola += individual[L] * 10
// 	cola += individual[A]
//
// 	oasis += individual[O] * 10000
// 	oasis += individual[A] * 1000
// 	oasis += individual[S] * 100
// 	oasis += individual[I] * 10
// 	oasis += individual[S]
//
// 	fitness := (coca + cola) - oasis
// 	if fitness < 0 {
// 		fitness = fitness * (-1)
// 	}
// 	individual[POS_EVAL] = fitness
...
// 	var donald, gerald, robert int
// 	donald += individual[D] * 100000
// 	donald += individual[O] * 10000
// 	donald += individual[N] * 1000
// 	donald += individual[A] * 100
// 	donald += individual[L] * 10
// 	donald += individual[D]
//
// 	gerald += individual[G] * 100000
// 	gerald += individual[E] * 10000
// 	gerald += individual[R] * 1000
// 	gerald += individual[A] * 100
// 	gerald += individual[L] * 10
// 	gerald += individual[D]
//
// 	robert += individual[R] * 100000
// 	robert += individual[O] * 10000
// 	robert += individual[B] * 1000
// 	robert += individual[E] * 100
// 	robert += individual[R] * 10
// 	robert += individual[T]
//
// 	fitness := (donald + gerald) - robert
// 	if fitness < 0 {
// 		fitness = fitness * (-1)
// 	}
// 	individual[POS_EVAL] = fitness
